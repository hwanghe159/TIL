...
1. 트랜잭션 전파
   ...
   - 트랜잭션 전파가 없다면 항상 새로운 트랜잭션을 시작하기 때문에 원하는 대로 작동하지 않아 기존 코드를 재사용할 수 없게 되어 코드의 중복이 일어날 것이다.
   - 트랜잭션 전파속성
...
스프링은 선언적 트랜잭션과 프로그램에 의한 트랜잭션 모두 지원한다.

- 선언적 트랜잭션 : AOP를 이용해 코드 외부에서 트랜잭션의 기능을 부여해주고 속성을 지정할 수 있게 하는 방법
- 프로그램에 의한 트랜잭션 : TransactionTemplate나 개별 데이터 기술의 트랜잭션 API를 사용해 직접 코드 안에서 사용하는 방법
- 특별한 경우 아니라면 선언적 방식을 사용하는 게 바람직하다
...
- 트랜잭션 매니저
  ...
  - 구체적인 트랜잭션 기술의 종류에 상관 없이 일관적인 트랜잭션 제어 가능

  - 아래와 같이 테스트코드에서 사용 가능

    ```java
    class UserServiceTest {
        @Autowired
        PlatformTransactionManager transactionManager;//트랜잭션 매니저가 빈으로 등록돼있기 때문에 autowired할수있음
...
- 트랜잭션 동기화
  ...
  - 시작된 트랜잭션 정보를 저장소에 보관해뒀다가 DAO에서 공유할 수 있음
  - 진행중인 트랜잭션이 있는지 확인하고 트랜잭션 전파 속성이 따라 이에 참여할 수 있도록 해준다

롤백 테스트

- 테스트 내의 모든 DB작업을 하나의 트랜잭션 안에서 동작하게 하고 테스트가 끝나면 무조건 롤백해버리는 테스트
- 테스트의 마지막에 finally문을 이용해서 롤백함으로써 구현 가능
- 장점
  - 테스트를 진행하는 동안에 조작한 데이터를 모두 롤백하고 테스트를 시작하기 전 상태로 만들어주기 때문에 다른 테스트들과의 격리를 보장할 수 있다
  - 여러 개발자가 하나의 공용 테스트용 DB를 사용할 수 있게 해준다

테스트코드에서의 @Transactional

- AOP를 위한 것은 아니고 컨텍스트 테스트 프레임워크에 의해 트랜잭션을 부여해주는 용도
- 테스트 메소드 실행 전에 새로운 트랜잭션을 만들어주고, 메소드가 종료되면 트랜잭션을 종료해준다
- 테스트가 끝나면 자동으로 롤백해준다. 자동 롤백을 원하지 않으면 메서드에 @Rollback(false)를 추가하면 된다. (클래스레벨엔 불가능) 모든 메서드에 롤백을 적용하지 않으려면 @TransactionalConfiguration(defaultRollback=false)를 클래스레벨에 부여하고, 롤백을 적용하고자 하는 메서드에는 따로 @Rollback을 붙이면 된다.
- @Transactional(readOnly=true)내부에서 DB에 변경을 일으키는 코드가 존재하면 예외가 발생한다
- 테스트 클래스 레벨에도 부여할 수 있다. 메소드의 트랜잭션 속성이 클래스의 속성보다 우선한다
- @Transactional(propagation=Propagation.NEVER)을 붙임으로써 트랜잭션을 적용하지 않을 순 있지만 트랜잭션 테스트와 비트랜잭션 테스트는 분리하는 게 권장된다.

<br/>

### 6.9 정리

- 트랜잭션 경계설정 코드를 분리해서 별도의 클래스로 만들고 비즈니스 로직 클래스와 동일한 인터페이스를 구현하면 DI의 확장기능을 이용해 클라이언트의 변경 없이도 깔끔하게 분리된 트랜잭션 부가기능을 만들 수 있다
- 트랜잭션처럼 환경과 외부 리소스에 영향을 받는 코드를 분리하면 비즈니스 로직에만 충실한 테스트를 만들 수 있다
- 목 오브젝트를 활용하면 의존관계속에 있는 오브젝트도 손쉽게 고립 테스트로 만들 수 있다
- DI를 이용한 트랜잭션의 분리는 데코레이터 패턴과 프록시 패턴으로 이해될 수 있다
- 번거로운 프록시 클래스 작성은 JDK의 다이내믹 프록시를 사용하면 간단하게 만들 수 있다
- 다이내믹 프록시는 정적 팩토리 메서드를 사용하기 때문에 빈으로 등록하기 번거롭다. 따라서 팩토리 빈으로 만들어야 한다, 스프링은 자동 프록시 생성 기술에 대한 추상화 서비스를 제공하는 프록시 팩토리 빈을 제공한다
- 프록시 팩토리 빈의 설정이 반복되는 문제를 해결하기 위해 자동프록시생성기와 포인트컷을 활용할 수 있다. 자동프록시생성기는 부가기능이 담긴 어드바이스를 제공하는 프록시를 스프링 컨테이너 초기화 시점에 자동으로 만들어준다
...
- AOP를 이용해 트랜잭션 속성을 지정하는 방법에는 포인트컷 표현식과 메소드 이름 패턴을 이용하는 방법과 타깃에 직접 부여하는 @Transactional을 사용하는 방법이 있다
...
- 트랜잭션 전파속성이 존재하는 이유중 하나가 코드의 중복을 줄이기 위해였다니