...
각 오브젝트와 메소드가 각각 자기 몫의 책임을 맡아 일을 하는 구조다. 오브젝트에게 데이터를 요구하지 말고 작업을 요청하는 객체지향적인 코드다.
...
- DAO의 메소드와 비즈니스 로직을 담고 있는 UserService의 메소드에 Connection 파라미터가 추가되야 한다. 싱글톤이기 때문에 필드에 저장할 수도 없다
...
- 트랜잭션 경계설정을 담당하는 코드는 일전한 패턴을 갖고 있기 때문에 추상화를 할 수 있다.
...
- 테스트 대상의 간접적은 출력 결과를 검증하고, 테스트 대상 오브젝트와 의존 오브젝트 사이에서 일어나는 일을 검증할수 있도록 특별히 설계된 오브젝트
...
- 이를 위해서는 DAO의 기술 변화에 서비스 계층의 코드가 영향을 받지 않도록 인터페이스와 DI를 잘 활용하여 결합도를 낮퉈줘야 한다.
