...
- `CompositeItemWriter`
  ...
  ![CompositeItemWriter](../images/CompositeItemWriter.jpeg)
...
- 프로파일링?
  ...
  - 프로파일링을 하면 최적화와 관련된 의사 결정을 내일 수 있으므로 성능을 높이는 데 긍정적인 영향을 줄 수 있다

- VisualVM

  - 배치 잡을 프로파일링할 수 있는 도구 중 하나 (무료)
  - JVM이 어떤 상태인지 파악할 수 있는 도구(JMX관리, CPU및 메모리 사용량, 메서드 실행 시간, 스레드 관리, GC정보 등등)

- 스프링 배치 애플리케이션 프로파일링 하기

  - 프로파일링할때 주로 보는 2가지
    1. 어떤 부분에서 얼마나 많은 CPU를 사용하는가? -> CPU 프로파일링
    2. 무엇 때문에 얼마나 많은 메모리가 사용되는가? -> 메모리 프로파일링

  - CPU 프로파일링
    - VisualVM의 Sampler 탭에서 CPU 실행 시간을 볼 수 있다
    - VisualVM을 사용하려 문제 위치를 좁혀나갈 수 있다
  - 메모리 프로파일링
    - 실제로는 CPU사용량보다는 메모리문자가 발생할 가능성이 더 높다
    - Monitor탭에서 메모리 사용량을 볼 수 있다
    - 메모리 사용량 급증의 원인을 모른다면 Sampler 탭이나 스냅샷 기능을 사용하여 유추할 수 있다
...
- 12요소 애플리케이션
  - 클라우드에서 애플리케이션 실행에 관련된 추가적인 문제를 해결해주는 방법론
  - 목표는 서비스 형태로 애플리케이션을 개발할 수 있는 패턴을 개발하는 것
  - 12요소
    1. 코드베이스 : 애플리케이션의 코드베이스는 하나의 버전 관리 저장소에 통합되어야 한다
    2. 의존성 : 모든 의존성은 애플리케이션 내에 캡슐화되어야 하고 명시적으로 선언되고 분리되어야 한다
    3. 구성 : 구성은 코드와 분리됨으로써 특정 환경에 의존적이면 안된다
    4. 백엔드 서비스 : 코드 내에서 특정 서비스 인스턴스에 직접적으로 의존성을 가지지 말고 URL등을 통하여 참조할 수 있어야 한다
    5. 빌드, 릴리즈, 실행 : 애플리케이션 빌드, 릴리즈 및 실행 프로세스를 엄격하게 구분한다
    6. 프로세스 : 확장성을 높이기 위해 상태를 저장하지 않으며 아무것도 공유하지 않는다.
    7. 포트 바인딩 : 애플리케이션을 서비스로 노출하기 위해 어떤 종류의 서버를 런타임에 추가할 필요가 없고 언타임이 포함돼 있으며 설계된 기능을 실행하는 데 필요한 경우 포트 자체에 포트바인딩 된다
    8. 동시성 : 인스턴스를 여러개로 늘려 사용해 확장할 수 있다. 또 JVM 내의 스레드를 통해 원격 청킹이나 원격 파티셔닝을 사용해 확장할 수 있다
    9. 폐기 기능 : 프로세스를 사전 통보 없이 중지했다가 재시작할 수 있도록 설계해야 한다. 중지요청을 받으면 종료되어야 한다
    10. 개발/운영 환경 일치 : 비즈니스 민첩성을 높이기 위해 지속적인 배포는 중요하다. 개발/운영 환경을 일치시킴으로써 릴리즈/배포 과정이 훨씬 원활해질 수 있다
    11. 로그 : 로그를 잘 수집하면 시스템 내 상황을 더 잘 이해할 수 있다
    12. 관리자 프로세스 : 관리 작업은 일회성 작업으로 실행해야 한다
