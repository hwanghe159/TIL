...
클래스 : "클라이언트야, 나는 ??이라는 인터페이스를 구현하고 있어. 그러니까 내 인스턴스로 할 수 있는 게 궁금하면 ?? 인터페이스를 보면 돼."
...
상수 인터페이스를 구현하는 것은 내부 구현을 API로 노출하는 행위 -> 클라이언트가 이 상수들에 종속되게 됨 -> 이 상수들이 더는 쓰이지 않더라도 이 상수 인터페이스를 구현하고 있어야 한다
