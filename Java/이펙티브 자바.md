...
1. 배타적 수행
   ...
   > "수정중이어서 일관성이 깨진 상태를 다른 스레드에서 보지 못하는가?"
   >
   > "수정이 완전히 반영된 값을 얻는가?"

2. 스레드 간 통신

   > "한 스레드가 저장한 값이 다른 스레드에게 보이는가?"

### 동기화 예시1

- 아래 코드는 메인 스레드가 백그라운드 스레드를 실행 하고 1초후 종료시키는 코드이다
...
public class StopThread {
  ...
  public static void main(String[] args) throws InterruptedException {
    Thread backgroundThread = new Thread(() -> {
      int i = 0;
      while (!stopRequested) { //이 루프는 영원히 끝나지 않는다
        i++;
      }
    });
...
- 예상과는 달리 루프는 영원히 끝나지 않는다.
- 메인 스레드가 수정한 `stopRequested`의 값을 백그라운드 스레드가 언제 볼지 알 수 없다
- `synchronized`키워드로 해결할 수 있다
...
- 읽기와 쓰기 모두 동기화하지 않으면 동작을 보장할 수 없다
- 아래 코드는 `volatile` 한정자를 이용해서 속도를 개선한 코드다 (`volatile` :  항상 최근에 기록된 값을 읽게 됨을 보장)
...
- 하지만 `++`연산은 여러 연산의 모음이다 (값 읽기 -> 읽은 값에 1 더하기 -> 더한 값을 저장)
- 연산 중 다른 스레드가 끼어든다면 안전 실패(safety failure) 발생
- 동시에 호출해도 서로 간섭하지 않고, 이전 호출이 변경한 값을 읽도록 `synchronized`사용

```java
private static long nextSerialNumber = 0; //메서드에 synchronized를 붙였으니까 volatile 제거

private static synchronized long generateSerialNumber() {
  if (nextSerialNumber == Long.MAX_VALUE) { //최대값이면 예외
    throw new ..
  }
  return nextSerialNumber++;
}
...
- volatile은 배타적 수행과 스레드간 통신 중 통신쪽만 지원하지만 이 패키지는 원자성(배타적 수행)까지 지원한다
- 성능도 동기화 버전보다 우수하다

### 팁

- 애초에 가변 데이터를 공유하지 않는 게 best
  - 불변 객체만 공유하거나 아무것도 공유x
  - 가변 객체는 단일 스레드에서만 쓰도록.
- 한 스레드가 데이터를 다 수정한 후 다른 스레드에 공유할 때 해당 객체에서 공유하는 부분만 동기화해도 됨
  - 이런 객체를 사실상 불변(effectively immutable)이라 하고 다른 스레드에 이런 객체를 건네는 행위를 안전 발행(safe publication)이라 함
