...
- 자바8 이전엔..
  ...
  2. null을 반환한다
     ...
     - null을 반환하는 메서드를 호출할 때는 별도의 null처리 코드를 추가해야 한다 -> 전혀 상관없는 코드에 NPE가 발생할 수 있다
...
1. 기본값을 정해둘 수 있다
   ...
   ```java
   //max()가 Optional<String>을 반환할때
   String longestWord = max(words).orElse("단어 없음...");
   ```

   ```java
   //기본값을 생성하는 비용이 큰 경우 orElseGet을 사용할 수도 있다
   String longestWord = max(words).orElseGet(() -> "단어 없음...");
   ```

2. 원하는 예외를 던질 수 있다

   ```java
   String longestWord = max(words).orElseThrow(RuntimeException::new);
   ```
...
3. 값이 채워있다고 가정하고 그냥 값을 꺼낸다
   ...
   ```java
   String longestWord = max(words).get();
   ```
