...
@Component을 붙인 클래스를 스캔해서 스프링 빈으로 등록하려면 아래처럼 설정 클래스에 @ComponentScan을 붙여야 한다.
...
- 프록시(proxy) : 핵심 기능의 실행은 다른 객체에게 위임하고 부가적인 기능을 제공하는 객체이다. 핵심 기능을 구현하지 않는 대신에 여러 객체에 공통으로 적용할 수 있는 기능을 구현한다. (예를 들어 핵심 기능이 계산하는 기능이라고 할때, 실행 시간 측정이나 캐시 등의 기능같은 공통적인 기능들을 구현한다.)
...
![KakaoTalk_20200610_013628251](C:\Users\junho\Desktop\KakaoTalk_20200610_013628251.jpg)
